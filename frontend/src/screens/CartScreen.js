...
const CartScreen = ({ match, location, history }) => {
  ...
  const checkoutHandler = () => {
    // if not logged in, go to log in, if they are logged in go to shipping
          ...
          <ListGroup variant="flush">
            {cartItems.map((item) => (
              <ListGroup.Item key={item.product}>
                <Row>
                  {/* assigned these properties in the cart action. item.product is the item id */}
                  <Col md={2}>
                    <Image
                      src={item.image}
                      alt={item.name}
      ...
      <Col md={4}>
        <Card>
          <ListGroup variant="flush">
            <ListGroup.Item>
              <h2>
                {/* read the reduce docs you lazy bum! */}
